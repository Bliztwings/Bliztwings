  ...
  <article>
    <section id='form' class="page-section" >
      <div class='panel' style="padding-left: 10px">
        <div class='panel-body' style="padding-left: 10px">
			<form id="formId" name="formId" class="form-horizontal" role="form" method='post' action="<%=basePath%>/resource/editMenuOrButton">
	...
	<div class="input-group" style="margin-bottom: 10px;">
      <span class="input-group-addon" style="width:150px;text-align: right;" >父菜单：</span>
        ...
        <c:if test="${level!=0}">
        <input type="text" name="dirName" id="dirName" value="${p_securityResource.name}" readonly class="form-control validate[required,maxSize[20]] required"/>
        </c:if> 
      ...
      <c:if test="${level==2}">
      	 <div class="input-group" style="margin-bottom: 10px;">
      		<span class="input-group-addon" style="width:150px;text-align: right;" >按钮名称：</span>
		        <input type="text" name="name" id="name" value="${securityResource.name}" class="form-control validate[required,maxSize[50]] required"/> 
		    <span class="glyphicon glyphicon-star"></span>
		 </div>
		 <div class="input-group" style="margin-bottom: 10px;">
		    <span class="input-group-addon" style="width:150px;text-align: right;" >权限代码：</span>
		        <input type="text" name="permission" id="permission" value="${securityResource.permission}" class="form-control validate[required,maxSize[50]] required"/> 
		    <span class="glyphicon glyphicon-star"></span>
		 </div>
      </c:if>
      <c:if test="${level!=2}">
      	    <div class="input-group" style="margin-bottom: 10px;">
	      		<span class="input-group-addon" style="width:150px;text-align: right;" >菜单名称：</span>
			        <input type="text" name="name" id="name" value="${securityResource.name}" class="form-control validate[required,maxSize[20]] required"/> 
			    <span class="glyphicon glyphicon-star"></span>
		    </div>
		    <div class="input-group" style="margin-bottom: 10px;">
		    	<span class="input-group-addon" style="width:150px;text-align: right;" >权限代码：</span>
		    ...
		    <div class="input-group" style="margin-bottom: 10px;">
		        ...
		        <input type="text" name="url" id="url" value="${securityResource.url}" class="form-control validate[required,maxSize[50]] required"/> 
		    	<span class="glyphicon glyphicon-star"></span>
		    </div>
		    </c:if>
		    ...
		    <c:if test="${level==0}">
		    <div class="input-group" style="margin-bottom: 10px;">
		    	<span class="input-group-addon" style="width:150px;text-align: right;" >菜单图标：</span>
		        <input type="text" name="icon" id="icon" value="${securityResource.icon}" class="form-control validate[required,maxSize[50]] required"/> 
		    	<span class="glyphicon glyphicon-star"></span>
		    </div>
		    </c:if>
		    
		    <div class="input-group" style="margin-bottom: 10px;">
		    	<span class="input-group-addon" style="width:150px;text-align: right;" >是否启用：</span>
		        <select id="available" name="available" class="form-control" style="width: 135px;">
		        	<option value="1" <c:if test="${securityResource.available==1}">selected</c:if>>启用</option>
		        	<option value="0" <c:if test="${securityResource.available==0}">selected</c:if>>禁用</option>
		        </select> 
		    	<span class="glyphicon glyphicon-star"></span>
		    </div>
      </c:if>
