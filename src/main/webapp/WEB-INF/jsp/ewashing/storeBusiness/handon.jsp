...
<script type="text/javascript">
	...
	function query(){
	    ...
	    location.href = "<%=basePath%>/storeBusiness/handon?"+params;
	}
	...
	head+='<th class="text-center" data-col-class="text-center" data-width="10%">挂衣区</th>';
	head+='<th class="text-center" data-col-class="text-center" data-width="10%">挂衣号</th>';
	head+='</tr>';
	head+='</thead>';
	head+='<tbody>';
	// 上挂
	...
	function confirmHandOn(){
		...
		$("#transLogQueryBtn").attr("disabled","disabled");//设置button不可用 ;
		...
		if(count==0){
			art.dialog.alert("请选择要上挂的衣服");
			return;
		}
		jQuery.ajax({
			url : "<%=basePath%>/storeBusiness/confirmHandOn",
			type : "post",
			cache : false,
			dataType : "json",
...
<body id="content">
	...
	<!-- 搜索条件 -->
	<form id="formId" name="formId" class="form-horizontal" role="form" method='get' action="<%=basePath%>/storeBusiness/handon">
					...
					<table class="table datatable table-bordered  table-hover" id="datatable">
						<thead>
							<tr>
								...
								<th class='text-center' data-col-class='text-center' data-width='10%'>已上挂件数</th>
								<th class='text-center' data-col-class='text-center' data-width='10%'>已上挂区</th>
								<th class='text-center' data-col-class='text-center' data-width='10%'>已上挂号</th>
							</tr>
						</thead>
						<!-- 同步表格 -->
						<tbody>
	 						<!--  PageHelper -->
							<c:forEach items="${clothesList}" var="handon">
								<tr>
									<td><input type="checkbox" name="checkbox" value="${handon.id}#${handon.orderCode}#${handon.barCode}#${handon.clothesName}#${handon.color}#${handon.brand}'"/></td>
									<td>${handon.orderCode}</td>
								    <td>${handon.barCode}</td>
								    <td>${handon.clothesName}</td>
								    <td>${handon.color}</td>
								    <td>${handon.brand}</td>
								    <td>${handon.clothesCount}</td>
								    <td>${handon.handOnCount}</td>
								    <td>${handon.handOnArea}</td>
								    <td>${handon.handOnNo}</td>
								</tr>
							</c:forEach>
						</tbody>
		...
		<div style="padding-left: 10px; margin-top: 5px;margin-bottom: 10px;">
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td width="60px" align="left" style="position:relative;padding-left: 10px;font-size: 12">挂衣区:</td>
					<td align="left">
						<select id="handonArea" onchange="chooseArea()" name="handonArea" class="form-control validate[required] required" style="width: 100px;">
			                <option value="0">挂衣区</option>
			                <c:forEach items="${areaList}" var="dto" varStatus="status">
			                <option value="${dto.id}">${dto.handOnArea}</option>
			                </c:forEach>
			            </select>
					</td>
					
					<td width="60px" align="left" style="position:relative;padding-left: 10px;font-size: 12">挂衣号:</td>
					<td align="left" id="handonNoHtml">
						<select id="handonNo" class="form-control validate[required,maxSize[16]] required" style="width: 80px;">
							<option value="0">挂衣号</option>
						</select>
					</td>
					
					<td align="left" style="position:relative;padding-left: 10px;">
						<button class="btn btn-primary" type="button" id="transLogQueryBtn" onclick="handon()" style="margin-right: 10px;">选定上挂衣服</button>
					</td>
					
					
					<!-- <td align="left" style="position:relative;padding-left: 10px;">
						<button class="btn btn-primary" type="button" id="transLogQueryBtn" onclick="handDown()" style="margin-right: 10px;">下  挂</button>
					</td> -->
	...
	<div style="padding-left: 10px; margin-top: 5px;margin-bottom: 10px;">
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<button class="btn btn-primary" type="button" id="transLogQueryBtn" onclick="confirmHandOn()" style="margin-right: 10px;">确认上挂</button>
			</tr>
		</table>
	</div>
</body>
