...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		//初始化已经编辑前数据
		$("#t_name").html('<font color=red>'+"【"+'${clothesName.clothesName }'+"】"+'</font>');
		$("#t_color").html('<font color=red>'+"【"+'${clothesName.color }'+"】"+'</font>');
		$("#t_brand").html('<font color=red>'+"【"+'${clothesName.brand }'+"】"+'</font>');
		
		// 附件、瑕疵有多个，需要单独处理
		$("#t_attach").html('<font color=red>'+'${attachNames}'+'</font>');
		$("#t_flaw").html('<font color=red>'+'${flawNames }'+'</font>');
		$("#attachList").val('${attachArrayList}');
	});
	...
	function getDate(tabNo,dataQueryKey){
		jQuery.ajax({
			...
			cache : false,
			dataType : "json",
			data : {'dataType':tabNo,'dataQueryKey':dataQueryKey},
				...
				if(data.dataList!=null){
					...
					for(var i=0;i<row;i++){
						...
						for(var j=0;j<array.length;j++){
							...
							// 初始化金额
							var clothesName=  '${clothesIds.clothesName}'
							if(clothesName ==array[j].id){
								var serviceType = $('input:radio[name="serviceType"]:checked').val();
								if(serviceType=='1'){
									$("#sumAmount").val(array[j].price);
								}
								
								if(serviceType=='2'){
									$("#sumAmount").val(array[j].commonPrice);
								}
								
								if(serviceType=='3'){
									$("#sumAmount").val(array[j].luxuriesPrice);
								}
							}
					...
					if(tabNo=='1'){
						...
						if(clothesName!=null && clothesName!=''){
							$("#" + clothesName).attr("style","border: 1px solid;border-color:#d3d3d3;width: 100px;height: 100px;text-align: center;vertical-align: middle;cursor: pointer;float: left; margin-top: 5px;line-height: 100px;  margin-left: 8px;background-color: rgb(194, 194, 194)");
						}
					}
					
					if(tabNo=='2'){
						// 选择颜色
						var color=  '${clothesIds.color}'
						if(color!=null && color!=''){
							$("#" + color).attr("style","border: 1px solid;border-color:#d3d3d3;width: 100px;height: 100px;text-align: center;vertical-align: middle;line-height: 100px;cursor: pointer;float: left; margin-top: 5px;  margin-left: 8px;background-color: rgb(194, 194, 194)");
						}
					}
					
					if(tabNo=='3'){
						// 选择品牌
						var brand=  '${clothesIds.brand}'
						if(brand!=null && brand!=''){
							$("#" + brand).attr("style","border: 1px solid;border-color:#d3d3d3;width: 100px;height: 100px;text-align: center;vertical-align: middle;cursor: pointer;float: left; margin-top: 5px;  margin-left: 8px;background-color: rgb(194, 194, 194)");
						}
					}
					
					if(tabNo=='4'){
						// 选择瑕疵
						var flawIds=  '${clothesIds.flaw}'
						if(flawIds!=null && flawIds!=''){
							var flaws =flawIds.split(",");
							for(var i=0;i<flaws.length;i++){
								$("#" + flaws[i]).attr("style","border: 1px solid;border-color:#d3d3d3;width: 100px;height: 100px;text-align: center;vertical-align: middle;line-height: 100px;cursor: pointer;float: left; margin-top: 5px;  margin-left: 8px;background-color: rgb(194, 194, 194)");
							}
						}
					}
					...
					if(tabNo=='5'){
						...
						if(attachIds!=null && attachIds!=''){
							var attachIdArray =attachIds.split(",");
							for(var i=0;i<attachIdArray.length;i++){
								$("#" + attachIdArray[i]).attr("style","border: 1px solid;border-color:#d3d3d3;width: 100px;height: 100px;text-align: center;vertical-align: middle;line-height: 100px;cursor: pointer;float: left; margin-top: 5px;  margin-left: 8px;background-color: rgb(194, 194, 194)");
							}
						}
						
						attachList=JSON.parse('${attachArrayList}'); 
					}
	...
	function remove(id) {
		...
		for (var i = 0; i < attachList.length; i++) {
			...
			if (id != attach.id) {
				attachListNew.push(attach);
			}
		}
		this.attachList=attachListNew;
	}
	// 选择服务类型
	...
	function updateClothes() {
		...
		jQuery.ajax({
			url : "<%=basePath%>/storeBusiness/updateClothes?"+params,
			type : "post",
			cache : false,
			dataType : "json",
			success : function(data, textStatus) {
...
<body id="content">
	...
	<!-- 搜索条件 -->
	<form id="formId" name="formId" class="form-horizontal" role="form" method='post' action="<%=basePath%>/storeBusiness/updateClothes">
		<div  style="padding-left: 10px; margin-top: 5px;">
			<input type="hidden" id="tabType"/>
			<input type="hidden" id="cardNumber" value="${clothesName.cardNumber}" name="cardNumber"/>
			<input type="hidden" id="memberId" value="${clothesName.memberId}" name="memberId"/>
			<input type="hidden" id="clothesName" value="${clothesIds.clothesName}" name="clothesName"/>
			<input type="hidden" id="color" value="${clothesIds.color}" name="color"/>
			<input type="hidden" id="brand" value="${clothesIds.brand}" name="brand"/>
			<input type="hidden" id="attachment" value="${attachIds}" name="attachment"/>
			<input type="hidden" id="attachList" value="" name="attachList"/>
			<input type="hidden" id="flaw" value="${clothesIds.flaw}" name="flaw"/>
			<input type="hidden" id="orderCode" value="${clothesName.orderCode}" name="orderCode"/>
			<input type="hidden" id="queryKey" value="${queryKey}" name="queryKey"/>
			<input type="hidden" id="id" value="${clothesIds.id}" name="id"/>
			...
			<table cellpadding="0" cellspacing="0" border="2" width="100%" style="border-color: #e8e8e8;" >
				<tr>
					...
					<td width="8%" align="left">
						<input type="text" style="margin-left: 5px;height: 30px" name="barCode" readonly="readonly"  value="${clothesName.barCode}" id="barCode"/>
					</td>
					...
					<td width="8%" align="left">
						<select name="urgency" id="urgency">
							<option value="0" <c:if test="${clothesName.urgency=='0'}">selected</c:if>>一般</option>
							<option value="1" <c:if test="${clothesName.urgency=='1'}">selected</c:if>>紧急</option>
						</select>
					</td>
					...
					<td width="100px" align="left">
						<input type="text" name="sumAmount" id="sumAmount" value="${clothesName.price }" style="height: 30px" value="0" />元
					</td>
				</tr>
				...
				<tr>
					...
					<td align="left" colspan="7">
					    &nbsp;<input type="radio" name="serviceType" <c:if test="${clothesName.serviceType=='2'}">checked="checked"</c:if>  onclick="chooseService()" value="2" style="vertical-align:middle; margin-top:0;"/> 普洗
				        &nbsp;<input type="radio" <c:if test="${clothesName.serviceType=='1'}">checked="checked"</c:if>  onclick="chooseService()"  name="serviceType" value="1" style="vertical-align:middle; margin-top:0;"/> 单烫&nbsp;
						<input type="radio" name="serviceType" <c:if test="${clothesName.serviceType=='3'}">checked="checked"</c:if> onclick="chooseService()" value="3" style="vertical-align:middle; margin-top:0;"/> 尚品洗涤&nbsp;
					</td>
				</tr>
				...
				<tr>
					...
					<td align="left" colspan="9">
						...
						<table style="padding-left: 2px;">
							<tr>
								<td>衣物名称:</td>
								<td id="t_name">${clothesName.clothesName }</td>
							</tr>
							<tr>
								<td>衣物颜色:</td>
								<td id="t_color">${clothesName.color }</td>
							</tr>
							<tr>
								<td>衣物品牌:</td>
								<td id="t_brand">${clothesName.brand }</td>
							</tr>
							<tr>
								<td>衣物瑕疵:</td>
								<td id="t_flaw"></td>
		...
		<div class="input-group" style="padding-left: 10px;margin-top: 10px;">
			<button class="btn btn-primary" type='button' onclick="updateClothes()" id="saveCloseBut" style="margin-right: 10px;">修  改</button>
		...
		<div style="position: relative;padding-left: 15px;margin-top: 10px;">
			<div style="float: left;">
				<input type="text" name="dataQueryKey" id ="dataQueryKey" style="height: 30px"/>
	...
	<div id="MyDiv" class="white_content">
		<div style="text-align: right; cursor: default; height: 100px;">
			...
			<div style="position: relative;margin-top: 10px;padding-right: 130px;">
				附件条码：<input type="text"  id="attachBarCode"/>
			</div>
