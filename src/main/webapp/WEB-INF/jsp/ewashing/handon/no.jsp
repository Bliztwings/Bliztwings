...
<script type="text/javascript">
	...
	function deleteHandonNo(id){
		art.dialog.confirm('确实要删除该挂衣号吗？', function () {
			jQuery.ajax({
				url : "<%=basePath%>/handon/deleteHandonNo",
				type : "post",
				cache : false,
				dataType : "json",
				data : {'id':id},
	...
	function query(){
	    ...
	    location.href = "<%=basePath%>/handon/no?"+params;
	}
	
	//  新增
	function add(obj){
		art.dialog.open( '<%=basePath%>/handon/initAddHandOnNo', {
			title : '添加挂衣号',
			width : 600,
			height : 300,
			lock : true,
...
<body id="content">
	...
	<!-- 搜索条件 -->
	<form id="formId" name="formId" class="form-horizontal" role="form" method='get' action="<%=basePath%>/handon/no">
		<div  style="padding-left: 10px; margin-top: 5px;">
			<table cellpadding="0" cellspacing="0" border="0" width="100%" >
				<tr>
					<td width="10%">
						<div class="input-group" >
							<span class="input-group-addon" style="width:20px; text-align: left;">挂衣区：</span> 
			                ...
			                <option value="${dto.id}" <c:if test="${area==dto.id}">selected</c:if>>${dto.handOnArea}</option>
			                </c:forEach>
			            </select>
						</div>
					</td>
					...
					<td width="10%">
						<div class="input-group" >
							<span class="input-group-addon" style="width:20px; text-align: left;">挂衣号：</span> 
							<input id="handOnNo" name="handOnNo" value="${handOnNo}" type="text" class="form-control" style="width: 150px;">
						</div>
					</td>
				...
				<table class="table  table-bordered  table-hover" id="datatable">
					<thead>
						<tr>
						    ...
						    <th class='text-center' data-col-class='text-center' data-width='10%' class='text-primary'>类型</th>
						    <th class='text-center' data-col-class='text-center' data-width='10%' class='text-primary'>所属门店</th>
							<th class='text-center' data-col-class='text-center' data-width='10%' class='text-primary'>挂衣区</th>
							<th class='text-center' data-col-class='text-center' data-width='10%' class='text-primary'>挂衣号</th>
					...
					<tbody>
						...
						<c:forEach items="${page.list}" var="handon">
							<tr>
							    <td class='text-center' style="display:none">${handon.id}</td>
							    <c:if test="${handon.handType=='1' }">
							    	<td class='text-center'>门店</td>
							    </c:if>
							    <c:if test="${handon.handType=='2' }">
							    	<td class='text-center'>工厂</td>
							    </c:if>
							    <c:if test="${handon.handType=='3' }">
							    	<td class='text-center'>萨维亚</td>
							    </c:if>
							    <c:if test="${handon.handType=='4' }">
							    	<td class='text-center'>浣衣坊</td>
							    </c:if>
							    <c:if test="${handon.handType==null || handon.handType=='' }">
							    ...
							    <c:if test="${handon.createDate ==null || handon.createDate ==''}">
							    	<td class='text-center'>&nbsp;</td>
							    </c:if>
							    
							    <c:if test="${handon.createDate !=null && handon.createDate !=''}">
							    	<td class='text-center'><fmt:formatDate value="${handon.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							    </c:if>
							    
							    <td class='text-center'>
							    	<a href="javascript:void(0)" onclick="deleteHandonNo('${handon.id}')">删除</a>
							    </td>
							</tr>
						</c:forEach>
					</tbody>
					...
					<tfoot>
						...
						<jsp:include page="../../inc/page_PageHelper.jsp">
								<jsp:param name="path" value="/handon" />
						</jsp:include></td>
					</tfoot>
