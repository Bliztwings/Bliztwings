...
function seach(){
    ...
    location.href = "<%=basePath%>/memberCardApply/approvedList?"+params;
}
...
function resetSeach(){
	$("#storeName").val();
	$("#storeNo").val();
	location.href = "<%=basePath%>/memberCardApply/approvedList";
}

function doApproved(id){
	art.dialog.open( '<%=basePath%>/memberCardApply/beforeApproved?id='+id, {
				title : '会员卡审批',
				width : 600,
				height : 450,
				lock : true,
...
<c:if test="${resultCode == 0}">
   <script type="text/javascript">
	...
	<!-- 搜索条件 -->
	<form id="formId" name="formId" class="form-horizontal" role="form" method='get' action="<%=basePath%>/memberCardApply/approvedList">
				...
				<table class="table datatable table-bordered table-striped table-hover" id="datatable">
					<thead>
						<tr>
							...
							<th data-width='50' style="text-align:center">操作</th>
						</tr>
					</thead>
					<!-- 同步表格 -->
					<tbody>
								...
								<td>
									<c:choose>
									   <c:when test="${dto.status=='un_approved' }">  
									   	<a href="javascript:void(0)" onclick="doApproved('${dto.id}')">审批</a>
									   </c:when>
									   <c:otherwise> 
									   </c:otherwise>
									</c:choose>
