  ...
  <article>
    <section id='form' class="page-section" >
      	<div class='panel' style="padding-left: 10px">
        	...
        	<form id="formId" name="formId" class="form-horizontal" role="form" method='post' action="<%=basePath%>/data/saveData" enctype="multipart/form-data">
		      ...
		      <span class="input-group-addon" style="width:150px;text-align: right;" >属性名称：</span>
		        <input id="dataName" name="dataName" value="${data.dataName }" type="text" class="form-control validate[required,maxSize[16]] required" style="width: 300px;"/>
		      <span class="glyphicon glyphicon-star"></span>
		    </div>
		    ...
		    <c:if test="${dataType=='1'}">
		    	...
		    	<div class="input-group" style="margin-bottom: 10px;">
				      <span class="input-group-addon" style="width:150px;text-align: right;" >单烫价格：</span>
				        <input id="price" name="price" value="${data.price }"  type="text" class="form-control validate[required,custom[onlyNumber]]" style="width: 300px;"/>
				      <span class="glyphicon glyphicon-star"></span>
		   		 </div>	
		   		 <div class="input-group" style="margin-bottom: 10px;">
				      <span class="input-group-addon" style="width:150px;text-align: right;" >普洗价格：</span>
				        <input id="commonPrice" name="commonPrice"  value="${data.commonPrice }" type="text" class="form-control validate[required,custom[onlyNumber]]" style="width: 300px;"/>
				      <span class="glyphicon glyphicon-star"></span>
		   		 </div>	
		   		 <div class="input-group" style="margin-bottom: 10px;">
				      <span class="input-group-addon" style="width:150px;text-align: right;" >尚品洗涤：</span>
				        <input id="luxuriesPrice" name="luxuriesPrice"  type="text" class="form-control validate[required,custom[onlyNumber]]" style="width: 300px;"/>
				      <span class="glyphicon glyphicon-star"></span>
		   		 </div>	
		   		 
		   		 <div class="input-group" style="margin-bottom: 10px;">
				      <span class="input-group-addon" style="width:150px;text-align: right;" >O2O价格：</span>
				        <input id="o2oPrice" name="o2oPrice"  type="text" class="form-control validate[required,custom[onlyNumber]]" style="width: 300px;"/>
				      <span class="glyphicon glyphicon-star"></span>
		   		 </div>	
		    ...
		    <div class="input-group" style="margin-bottom: 10px;">
		      <span class="input-group-addon" style="width:150px;text-align: right;" >属性类型：</span>
		       <select id="dataTypeSelect" name="dataType" class="form-control validate[required,maxSize[16]] required" style="width: 300px;">
		       		<option value="1" <c:if test="${dataType=='1'}">selected</c:if>>衣物名称</option>
		       		<option value="2" <c:if test="${dataType=='2'}">selected</c:if>>颜色</option>
		       		<option value="3" <c:if test="${dataType=='3'}">selected</c:if>>品牌</option>
		       		<option value="4" <c:if test="${dataType=='4'}">selected</c:if>>瑕疵</option>
		       		<option value="5" <c:if test="${dataType=='5'}">selected</c:if>>附件</option>
		       </select>
		      <span class="glyphicon glyphicon-star"></span>
		    </div>
