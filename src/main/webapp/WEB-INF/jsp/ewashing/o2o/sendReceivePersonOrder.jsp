...
<script type="text/javascript">
	...
	function query(){
	    ...
	    location.href = encodeURI("<%=basePath%>/o2oOrder/list?"+params);
	}
	...
	function distribute(){
		...
		jQuery.ajax({
			...
			cache : false,
			dataType : "json",
			data : {'ids':arrays,'sendUserId':sendUserId,type:'1'},
				...
				<tr>
					...
					<td width="10%">
						<div class="input-group" >
							...
							<select name ="distributeStatus" class="form-control"  style="width: 160px;">
							    <option value="">全部</option>
							    <option value="0" <c:if test="${distributeStatus=='0'}">selected</c:if>>未分配</option>
							    <option value="2" <c:if test="${distributeStatus=='2'}">selected</c:if>>已分配</option>
							</select>
						</div>
					</td>
						...
						<c:forEach items="${page.list}" var="order">
							<tr>
								<td class='text-center'>
									<c:if test="${order.distributeStatus=='0' }">
							    ...
							    <td class='text-center'>
							    	<c:if test="${order.distributeStatus=='2' }">
